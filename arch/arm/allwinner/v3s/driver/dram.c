...
static void mctl_await_completion(uint32_t * reg, uint32_t mask, uint32_t val)
{
	...
	while((read32((uint32_t)reg) & mask) != val)
	{
		if(timeout++ > 10000)
			break;
	}
...
static void mctl_phy_init(uint32_t val)
{
	...
	write32((uint32_t)&ctl->pir, val | PIR_INIT);
...
static void mctl_dq_delay(uint32_t read, uint32_t write)
{
	...
	for(i = 0; i < 4; i++)
	{
		...
		for(j = DATX_IOCR_DQ(0); j <= DATX_IOCR_DM; j++)
			write32((uint32_t)&ctl->datx[i].iocr[j], val);
	}
	...
	for(i = 0; i < 4; i++)
	{
		...
		write32((uint32_t)&ctl->datx[i].iocr[DATX_IOCR_DQS], val);
		write32((uint32_t)&ctl->datx[i].iocr[DATX_IOCR_DQSN], val);
	}
...
static void mctl_set_master_priority(void)
{
	...
	write32((uint32_t)&com->bwcr, 0x00010190);
	write32((uint32_t)&com->mapr, 0x00000001);
	write32((uint32_t)&com->mcr[0][0], 0x0200000d);
	write32((uint32_t)&com->mcr[0][1], 0x00800100);
	write32((uint32_t)&com->mcr[1][0], 0x06000009);
	write32((uint32_t)&com->mcr[1][1], 0x01000400);
	write32((uint32_t)&com->mcr[2][0], 0x0200000d);
	write32((uint32_t)&com->mcr[2][1], 0x00600100);
	write32((uint32_t)&com->mcr[3][0], 0x0100000d);
	write32((uint32_t)&com->mcr[3][1], 0x00200080);
	write32((uint32_t)&com->mcr[4][0], 0x07000009);
	write32((uint32_t)&com->mcr[4][1], 0x01000640);
	write32((uint32_t)&com->mcr[5][0], 0x0100000d);
	write32((uint32_t)&com->mcr[5][1], 0x00200080);
	write32((uint32_t)&com->mcr[6][0], 0x01000009);
	write32((uint32_t)&com->mcr[6][1], 0x00400080);
	write32((uint32_t)&com->mcr[7][0], 0x0100000d);
	write32((uint32_t)&com->mcr[7][1], 0x00400080);
	write32((uint32_t)&com->mcr[8][0], 0x0100000d);
	write32((uint32_t)&com->mcr[8][1], 0x00400080);
	write32((uint32_t)&com->mcr[9][0], 0x04000009);
	write32((uint32_t)&com->mcr[9][1], 0x00400100);
	write32((uint32_t)&com->mcr[10][0], 0x2000030d);
	write32((uint32_t)&com->mcr[10][1], 0x04001800);
	write32((uint32_t)&com->mcr[11][0], 0x04000009);
	write32((uint32_t)&com->mcr[11][1], 0x00400120);
}
...
static void mctl_set_timing_params(struct dram_para_t * para)
{
	...
	write32((uint32_t)&ctl->mr[0], 0x263);		/* CL=11, WR=12 */
	write32((uint32_t)&ctl->mr[1], 0x4);
	write32((uint32_t)&ctl->mr[2], 0x0);			/* CWL=8 */
	write32((uint32_t)&ctl->mr[3], 0x0);
 
	/* Set DRAM timing */
	write32((uint32_t)&ctl->dramtmg[0], DRAMTMG0_TWTP(twtp) | DRAMTMG0_TFAW(tfaw) | DRAMTMG0_TRAS_MAX(trasmax) | DRAMTMG0_TRAS(tras));
	write32((uint32_t)&ctl->dramtmg[1], DRAMTMG1_TXP(txp) | DRAMTMG1_TRTP(trtp) | DRAMTMG1_TRC(trc));
	write32((uint32_t)&ctl->dramtmg[2], DRAMTMG2_TCWL(tcwl) | DRAMTMG2_TCL(tcl) | DRAMTMG2_TRD2WR(trd2wr) | DRAMTMG2_TWR2RD(twr2rd));
	write32((uint32_t)&ctl->dramtmg[3], DRAMTMG3_TMRW(tmrw) | DRAMTMG3_TMRD(tmrd) | DRAMTMG3_TMOD(tmod));
	write32((uint32_t)&ctl->dramtmg[4], DRAMTMG4_TRCD(trcd) | DRAMTMG4_TCCD(tccd) | DRAMTMG4_TRRD(trrd) | DRAMTMG4_TRP(trp));
	write32((uint32_t)&ctl->dramtmg[5], DRAMTMG5_TCKSRX(tcksrx) | DRAMTMG5_TCKSRE(tcksre) | DRAMTMG5_TCKESR(tckesr) | DRAMTMG5_TCKE(tcke));
 
	/* Set two rank timing */
	clrsetbits_le32(&ctl->dramtmg[8], (0xff << 8) | (0xff << 0), (0x66 << 8) | (0x10 << 0));
 
	/* Set PHY interface timing, write latency and read latency configure */
	write32((uint32_t)&ctl->pitmg[0], (0x2 << 24) | (t_rdata_en << 16) | (0x1 << 8) | (wr_latency << 0));
 
	/* Set PHY timing, PTR0-2 use default */
	write32((uint32_t)&ctl->ptr[3], PTR3_TDINIT0(tdinit0) | PTR3_TDINIT1(tdinit1));
	write32((uint32_t)&ctl->ptr[4], PTR4_TDINIT2(tdinit2) | PTR4_TDINIT3(tdinit3));
 
	/* Set refresh timing */
	write32((uint32_t)&ctl->rfshtmg, RFSHTMG_TREFI(trefi) | RFSHTMG_TRFC(trfc));
}
...
static void mctl_zq_calibration(struct dram_para_t * para)
{
	...
	if(((read32(0x01c00000 + 0x24) & 0xff) == 0) && ((read32(0x01c00000 + 0xf0) & 0x1) == 0))
	{
		...
		write32((uint32_t)&ctl->pir, PIR_CLRSR);
		mctl_phy_init(PIR_ZCAL);
 
		val = read32((uint32_t)&ctl->zqdr[0]);
		val &= (0x1f << 16) | (0x1f << 0);
		val |= val << 8;
		write32((uint32_t)&ctl->zqdr[0], val);
 
		val = read32((uint32_t)&ctl->zqdr[1]);
		val &= (0x1f << 16) | (0x1f << 0);
		val |= val << 8;
		write32((uint32_t)&ctl->zqdr[1], val);
		write32((uint32_t)&ctl->zqdr[2], val);
	}
	else
	{
		write32((uint32_t)&ctl->zqdr[2], 0x0a0a0a0a);
 
		for(i = 0; i < 4; i++)
		{
			zq = (CONFIG_DRAM_ZQ >> (i * 4)) & 0xf;
			write32((uint32_t)&ctl->zqcr, (zq << 20) | (zq << 16) | (zq << 12) | (zq << 8) | (zq << 4) | (zq << 0));
 
			write32((uint32_t)&ctl->pir, PIR_CLRSR);
			mctl_phy_init(PIR_ZCAL);
 
			zq_val[i] = read32((uint32_t)&ctl->zqdr[0]) & 0xff;
			write32((uint32_t)&ctl->zqdr[2], REPEAT_BYTE(zq_val[i]));
 
			write32((uint32_t)&ctl->pir, PIR_CLRSR);
			mctl_phy_init(PIR_ZCAL);
 
			val = read32((uint32_t)&ctl->zqdr[0]) >> 24;
			zq_val[i] |= bin_to_mgray(mgray_to_bin(val) - 1) << 8;
		}
 
		write32((uint32_t)&ctl->zqdr[0], (zq_val[1] << 16) | zq_val[0]);
		write32((uint32_t)&ctl->zqdr[1], (zq_val[3] << 16) | zq_val[2]);
	}
}
...
static void mctl_set_cr(struct dram_para_t * para)
{
	...
	write32((uint32_t)&com->cr, MCTL_CR_BL8 |
		MCTL_CR_2T |
...
static void mctl_sys_init(struct dram_para_t * para)
{
	...
	val = read32(CCU_MBUS_CLK);
	val &= ~(0x1 << 31);
	write32(CCU_MBUS_CLK, val);
 
	val = read32(CCU_MBUS_RST);
	val &= ~(0x1 << 31);
	write32(CCU_MBUS_RST, val);
 
	val = read32(CCU_BUS_CLK_GATE0);
	val &= ~(0x1 << 14);
	write32(CCU_BUS_CLK_GATE0, val);
 
	val = read32(CCU_BUS_SOFT_RST0);
	val &= ~(0x1 << 14);
	write32(CCU_BUS_SOFT_RST0, val);
 
	val = read32(CCU_PLL_DDR0_CTRL);
	val &= ~(0x1 << 31);
	write32(CCU_PLL_DDR0_CTRL, val);
	sdelay(10);
 
	val = read32(CCU_DRAM_CLK_GATE);
	val &= ~(0x1 << 31);
	write32(CCU_DRAM_CLK_GATE, val);
	sdelay(1000);
 
	clock_set_pll_ddr(CONFIG_DRAM_CLK * 2 * 1000000);
 
	val = read32(CCU_DRAM_CFG);
	val &= ~(0xf << 0);
	val &= ~(0x3 << 20);
	val |= ((1 - 1) << 0);
	val |= (0x0 << 20);
	val |= (0x1 << 16);
	write32(CCU_DRAM_CFG, val);
	mctl_await_completion((uint32_t *)(CCU_DRAM_CFG), 0x1 << 16, 0);
	...
	write32((uint32_t)&ctl->clken, 0xc00e);
	sdelay(500);
}
...
static int mctl_channel_init(struct dram_para_t * para)
{
	...
	write32((uint32_t)&com->protect, 0x94be6fa3);
	sdelay(100);
	clrsetbits_le32(&ctl->upd2, 0xfff << 16, 0x50 << 16);
	write32((uint32_t)&com->protect, 0x0);
	sdelay(100);
	...
	{
		write32((uint32_t)&ctl->datx[2].gcr, 0x0);
		write32((uint32_t)&ctl->datx[3].gcr, 0x0);
	}
	...
	if(read32((uint32_t)&ctl->pgsr[0]) & (0xfe << 20))
	{
		if(((read32((uint32_t)&ctl->datx[0].gsr[0]) >> 24) & 0x2) || ((read32((uint32_t)&ctl->datx[1].gsr[0]) >> 24) & 0x2))
		{
			clrsetbits_le32(&ctl->dtcr, 0xf << 24, 0x1 << 24);
			para->dual_rank = 0;
		}
 
		if(((read32((uint32_t)&ctl->datx[2].gsr[0]) >> 24) & 0x1) || ((read32((uint32_t)&ctl->datx[3].gsr[0]) >> 24) & 0x1))
		{
			write32((uint32_t)&ctl->datx[2].gcr, 0x0);
			write32((uint32_t)&ctl->datx[3].gcr, 0x0);
			para->bus_width = 16;
	...
	write32((uint32_t)&ctl->pgcr[3], 0x00aa0060);
	setbits_le32(&ctl->zqcr, ZQCR_PWRDOWN);
	write32((uint32_t)&com->maer, 0xffffffff);
...
void sys_dram_init(void)
{
	...
	if(para.dual_rank)
		write32((uint32_t)&ctl->odtmap, 0x00000303);
	else
		write32((uint32_t)&ctl->odtmap, 0x00000201);
	sdelay(1);
	write32((uint32_t)&ctl->odtcfg, 0x04000400);
