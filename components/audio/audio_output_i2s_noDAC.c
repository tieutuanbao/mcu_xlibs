...
static inline void i2s_make_sample_stereo32(i2s_no_dac_t *dev, int32_t *sample_r,  int32_t *sample_l){
    if(dev->i2s_config.bits_per_sample == 8) {
        sample_r[0] = ((sample_r[0] & 0xFF) - 128) << 8 ;
        sample_l[0] = ((sample_l[0] & 0xFF) - 128) << 8 ;
    }
}
...
audio_output_stt_t i2s_no_dac_consume_sample(i2s_no_dac_t *dev, int32_t *sample, uint16_t num_sample) {
    ...
    int32_t temp_sample[num_sample * 2];
    ...
    for(uint16_t index_sample = 0; index_sample < num_sample; index_sample++) {
        ...
        // i2s_make_sample_stereo32(dev, &temp_sample[index_sample * 2], &temp_sample[index_sample * 2 + 1]);
    }
