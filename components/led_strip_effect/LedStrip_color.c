...
Color_RGB_t LEDStripEffect_getColorGradient(uint32_t colorAt, uint32_t resolution, Color_RGB_t *colorStrip, uint8_t colorCount) {
    ...
    uint32_t maxFrame = resolution / (colorCount - 1); // số lượng frame mỗi khoảng màu
    uint32_t currentFrame = colorAt % (maxFrame + 1); // chỉ mục của frame ở mỗi khoảng màu
    uint16_t startColorIndex = colorAt / (maxFrame + 1);
    ...
    Color_RGB_t ret;
    int16_t rateBrightness = currentFrame * 100 / maxFrame;
