...
bool ICACHE_FLASH_ATTR i2s_set_bits(int bits)
{
...
uint16_t ICACHE_FLASH_ATTR i2s_available()
{
    return _i2s_available(tx);
}

uint16_t ICACHE_FLASH_ATTR i2s_rx_available()
{
...
static bool ICACHE_FLASH_ATTR _alloc_channel(i2s_state_t *ch)
{
    ...
    {
        ch->slc_buf_pntr[x] = (uint32_t *)os_malloc(SLC_BUF_LEN * sizeof(ch->slc_buf_pntr[0][0]));
        os_memset(ch->slc_buf_pntr[x], 0, SLC_BUF_LEN * sizeof(ch->slc_buf_pntr[0][0]));
...
uint16_t ICACHE_FLASH_ATTR i2s_write_buffer_mono_nb(const int16_t *frames, uint16_t frame_count) { return _i2s_write_buffer(frames, frame_count, true, true); }

uint16_t ICACHE_FLASH_ATTR i2s_write_buffer_mono(const int16_t *frames, uint16_t frame_count) { return _i2s_write_buffer(frames, frame_count, true, false); }

uint16_t ICACHE_FLASH_ATTR i2s_write_buffer_nb(const int16_t *frames, uint16_t frame_count) { return _i2s_write_buffer(frames, frame_count, false, true); }
...
bool ICACHE_FLASH_ATTR i2s_read_sample(int16_t *left, int16_t *right, bool blocking)
{
    if (!rx)
    {
        return false;
...
void ICACHE_FLASH_ATTR i2s_set_rate(uint32_t rate)
{ // Rate in HZ
...
bool ICACHE_FLASH_ATTR i2s_rxtx_begin(bool enableRx, bool enableTx)
{
...
bool ICACHE_FLASH_ATTR i2s_rxtxdrive_begin(bool enableRx, bool enableTx, bool driveRxClocks, bool driveTxClocks)
{
    if (tx || rx)
    {
        i2s_end(); // Stop and os_free any ongoing stuff
    }

    if (enableTx)
    {
        tx = (i2s_state_t *)os_malloc(1 * sizeof(*tx));
        os_memset(tx, 0, sizeof(*tx));
        if (!tx)
        {
        ...
        esp_gpio_config(I2SO_DATA, GPIO_MODE_F1);
        if (driveTxClocks)
        {
            esp_gpio_config(I2SO_WS, GPIO_MODE_F1);
            esp_gpio_config(I2SO_BCK, GPIO_MODE_F1);
        }
    }
    if (enableRx)
    {
        rx = (i2s_state_t *)os_malloc(1 * sizeof(*rx));
        os_memset(rx, 0, sizeof(*rx));
        if (!rx)
        {
        ...
        if (driveRxClocks)
        {
            esp_gpio_config(I2SI_WS, GPIO_MODE_OUT_PP);
            esp_gpio_config(I2SI_BCK, GPIO_MODE_OUT_PP);
